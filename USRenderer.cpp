...
void USRenderer::getTransducer(){
    //std::cout << "C_pos:" << uslaunchParams.transducer.position.x << " " << uslaunchParams.transducer.position.y << " " << uslaunchParams.transducer.position.z << std::endl;
    std::cout << "dir:"<< uslaunchParams.transducer.direction <<  std::endl;
}

vec3f rotateVector(const vec3f& vec, float angle, const vec3f& axis) {
    float rad = angle * M_PI / 180.0; // 将角度转换为弧度
