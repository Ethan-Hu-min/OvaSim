...
extern "C" __global__ void __closesthit__radiance()
{
    ...
    //vec3f world_raypos = ray_orig + t_hit * rayDir;
...
{
    vec3f& prd = *(vec3f*)getPRD<vec3f>();
    // set to constant white as background color 173, 113, 104
    prd = vec3f(0.67f,0.44f,0.40f);
}
...
extern "C" __global__ void __raygen__renderFrame()
{
    ...
    //if (iy == optixLaunchParams.frame.size.y / 2) {
    //    printf("%f %f %f\n", camera.position.x, camera.position.y, camera.position.z);
    //}
    //printf("%d \n", int(optixLaunchParams.traversable));
